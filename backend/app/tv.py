...
def stage_search(cfg=None):
    ...
    why = hold_reason(cfg)
    if why:
        core.log(f"tv search: holding off ({why})"); return
...
def _plex_ep_refresh(ep, cfg):
    """Refresh + analyze this episode on every PMS (master + replica) and in BOTH libraries that
    hold it — Series/Anime and their -EN mirrors. A plain scan won't re-read streams after an
    in-place remux, and the -EN entry points at the same file, so it needs analysing too."""
    from .pipeline import plex_refresh, mirror_to_en
    try:
        folder = os.path.dirname(ep["french_path"]).replace(cfg["media_mount"], cfg["plex_media_prefix"], 1)
        en_dir = mirror_to_en(ep["french_path"], cfg)     # ensure the -EN symlink exists first
        plex_refresh(cfg, [folder, en_dir], ep["series_title"],
